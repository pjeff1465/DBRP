...
\begin{enumerate}
    ...
    \begin{figure}[htbp]  % 'h' places the figure "here" (can be adjusted)
        \centering
        \includegraphics[width=\textwidth]{sa.jpeg}  % Adjust width as needed
    ...
    \begin{figure}[htbp]  % 'h' places the figure "here" (can be adjusted)
        \centering
        \includegraphics[width=\textwidth]{SA_ex.jpeg}  % Adjust width as needed
    ...
    \begin{figure}[htbp]  % 'h' places the figure "here" (can be adjusted)
        \centering
        \includegraphics[width=\textwidth]{SSA.jpeg}  % Adjust width as needed
...
\begin{figure}[htbp]  % 'h' places the figure "here" (can be adjusted)
    \centering
    \includegraphics[width=\textwidth]{SSA_ex.jpeg}  % Adjust width as needed
...
\begin{figure}[htbp]  % 'h' places the figure "here" (can be adjusted)
    \centering
    \includegraphics[width=\textwidth]{mmi.jpeg}  % Adjust width as needed
    \caption{This illustration represents the property of MMI in the boundary/ bulk. Where S(AC) = S(A U C)}  % Caption text
    \label{fig:mmi}  % Label for referencing
\end{figure}

This property represents an additional constraint beyond SSA for holographic entanglement entropy. It ensures the fact that mutual information shared between three regions cannot be arbitrarily distributed.  
\\ 
\indent This is important because in holographic systems that have gravitational duals, the way information is distributed must conform to the underlying spacetime geometry. This property ensures the geometric encoding is consistent with the rules of the AdS/CFT correspondence. 
\\
We can represent this property on our example from above (Fig. 3). 
\\
S(A) = 4, S(B) = 3, S(C) = 4, S(AB) = 5, S(BC) = 5, S(AC) = 6, S(ABC) = 7
\\
\[
5 + 5 + 6 \geq 4 + 3 + 4 + 7
\]
